...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov  3 01:47:39 2023
...
def recsys_rerank():
    ...
    if userintrain:
        print("YES PRESENT!!!!")
        diversity_metrics = pd.read_csv('/home/rockwell/diversity_metrics_latest_data.csv')
        domains_diversity = diversity_metrics['domains'].unique().tolist()
        ...
        for index,row in pd_hometimeline_urls.iterrows():
            if row['tagged_urls'] != 'NA':
                try:
                    ...
                    diversity_rating = 0
                    if row['tagged_urls'] in domains_diversity:
                        diversity_value = diversity_metrics.loc[diversity_metrics['domains'] == row['tagged_urls']]['variance'].values[0]
                        diversity_rating = (1.0 - math.exp(-1.0*(diversity_value - 5.0)))/(1.0 + math.exp(diversity_value - 5.0))
                    predicted_rating[row['tweet_id']] = alpha_m*recsys_rating + alpha_t*row['rating_age'] + diversity_rating
        ...
        for index,row in pd_hometimeline_authors.iterrows():
            ...
            if row['tagged_authors'] != 'NA':
                try:
                    ...
                    diversity_rating = 0
                    if row['tagged_authors'] in domains_diversity:
                        diversity_value = diversity_metrics.loc[diversity_metrics['domains'] == row['tagged_authors']]['variance'].values[0]
                        diversity_rating = (1.0 - math.exp(-1.0*(diversity_value - 5.0)))/(1.0 + math.exp(diversity_value - 5.0))
                    predicted_rating[row['tweet_id']] = alpha_m*recsys_rating + alpha_t*row['rating_age'] + diversity_rating
                    #predicted_rating[row['tweet_id']] = algo.predict(uid=screen_name, iid=row['tagged_urls']).est
                except ValueError:
