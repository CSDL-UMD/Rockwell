...
class ContentItem(BaseModel):
    ...
    body: str = Field(
        description="The body of the item. Assume UTF-8, and that leading and trailing whitespace have been trimmed."
    )

    actor_name: str = Field(
        description="This is the actor name. Use this to determine which posts are by the same author. When the post is by the current user, this should match `session.user_name_hash`."
    )
    actor_username: str = Field(
        description="The actor username. Use this to determine which posts are by the same author. When the post is by the current user, this should match `session.user_name_hash`."
    )
    actor_picture: str = Field(
        description="This is the actor picture. Use this to determine which posts are by the same author. When the post is by the current user, this should match `session.user_name_hash`."
    )

    # I think we are only doing posts
    ...
    embeded_image: Optional[HttpUrl] = Field(
